...
def get_notifications():
    ...
    for n in notifications.items:
        ...
        if n.related_project_id and n.project:
            project_info = {
                'id': n.project.id,
                'name': n.project.name
            }
        
        if n.related_task_id and n.task:
            task_info = {
                'id': n.task.id,
                'title': n.task.title
            }
...
def create_notification_for_members(project_id, notification_type, title, content, 
                                   exclude_user_id=None, task_id=None, include_self=False):
    """
    為專案成員批量建立通知（內部函數）
    ...
    參數:
        ...
        title: 通知標題
        content: 通知內容
        exclude_user_id: 要排除的使用者 ID（通常是操作者）
        task_id: 相關任務 ID（可選）
    ...
    for user_id in user_ids:
        # 排除特定使用者（除非 include_self 為 True）
        if not include_self and exclude_user_id and user_id == exclude_user_id:
            continue
